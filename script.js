...
function searchPet(){
  ...
  for(var i=0;i<salon.pets.length;i++){
    var selected= salon.pets[i];
    if(selected.petName.toLowerCase().includes(searchString) || selected.petService.toLowerCase().includes(searchString)){
      $('#'+selected.id).addClass('active2');
    }
    else{
      $('#'+selected.id).removeClass('active2');
    }
  }
...
function init(){
    ...
    //keypress function that allows you to search when you hit enter on a field
